...
import { AmaraAgentVoiceEnabled } from '../../lib/agents/amara-agent-voice-enabled';
import { OrdaClient } from '../../lib/orda/orda-client';

describe('AMARA Voice + ORDA End-to-End (Real API)', () => {
  let amaraAgent: AmaraAgentVoiceEnabled;
  ...
  beforeAll(() => {
    ...
    amaraAgent = new AmaraAgentVoiceEnabled({
      ordaClient
    });
  });
  ...
  it('should process voice query and create publicly verifiable testament', async () => {
    ...
    const publicTestament = await verifyResponse.json() as any;
