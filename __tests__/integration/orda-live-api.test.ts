...
describe('ORDA Live API Integration', () => {
  ...
  describe('Testament Anchoring (Real API)', () => {
    ...
    it('should handle ORDA API errors gracefully', async () => {
      const invalidTestament: Testament = {
        id: 'invalid-test',
        said: 'invalid-said',
      ...
      await expect(
        ordaClient.anchorTestament(invalidTestament)
      ).rejects.toThrow('ORDA API Error');
    }, 10000);
