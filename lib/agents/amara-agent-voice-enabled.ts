...
import { Testament } from '../testament-protocol';
...
export class AmaraAgentVoiceEnabled {
  private ordaClient: OrdaClient;
   ...
   */
  async handleUserQuery(input: UserQuery, tier: string): Promise<AmaraResponse> {
    try {
   ...
   */
  private evaluateConstitutionalGates(input: string): Record<string, boolean> {
   ...
   */
  private generateResponse(input: string, language: string): string {
   ...
   */
  private isValidInteraction(input: string): boolean {
   ...
   * Compliance check
   */
  private meetsComplianceRequirements(input: string): boolean {
